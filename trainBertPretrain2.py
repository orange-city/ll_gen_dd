...
class Encoder(nn.Module):
    ...
    def __init__(self, labelSize):
        ...
        self.bert = BertModel.from_pretrained('transformers/examples/chatgpt_pre4000_bert_82')
...
def run_epoch(data_iter, model, loss_compute,train=True,id2label=None):
    ...
    for i, (sent_batch,tag_batch) in enumerate(data_iter):
        ...
        if not train:
            ...
            predicted_labels = [1 if prob > threshold else 0 for prob in results]
            # print("1")
...
# def filter_data(sentences, model, tokenizer, label2id, threshold=0.9):
#     filtered_sentences = []
#     for sent in sentences:
#         input_ids, attention_mask, label_ids, lengths = idData(tokenizer, [sent], label2id)
...
#         confidence = torch.max(prediction_probs, dim=-1).values
#         is_good_sentence = confidence.item() >= threshold
#
#         if is_good_sentence:
...
def convert_to_features(tokenizer, sents, label2id):
    all_input_ids = []
    all_attention_masks = []
    all_tags = []
    for sent in sents:
        input_ids, attention_mask, tags, _ = idData(tokenizer, [sent], label2id)  # 注意这里需要使用下划线来接收多余的返回值
        all_input_ids.extend(input_ids[0])
        all_attention_masks.extend(attention_mask[0])
    ...
    tags_tensor = torch.tensor(all_tags, dtype=torch.long)

    return input_ids_tensor, attention_mask_tensor, tags_tensor

def filter_data(sents, encoder, tokenizer, label2id, threshold=0.5):
    input_ids, attention_mask, tags = convert_to_features(tokenizer, sents, label2id)

    with torch.no_grad():
        input_ids = input_ids.unsqueeze(0).to(device)                # 增加一维并移动到指定设备
        attention_mask = attention_mask.unsqueeze(0).to(device)      # 增加一维并移动到指定设备

        outputs = encoder(input_ids, attention_mask)    # 使用encoder获取输出

    logits = outputs.logits[0]                        # 获取模型输出的logits

    # 假设你需要根据阈值threshold筛选出符合条件的数据，你可以在这里进行处理
    filtered_sents = []
    for i in range(len(sents)):
        if logits[i][0] > threshold:  # 假设logits[i][0]是负例的logit
...
tokenizer = BertTokenizer.from_pretrained('transformers/examples/chatgpt_pre4000_bert_82', do_lower_case=False)


t=readData('dps/swbd/fake_data_1K')
...
print("ID finished")
check_point = {}
filtered_train_sents = filter_data(t, encoder, tokenizer, label2id, threshold=threshold)
print("Number of filtered training sentences:", len(filtered_train_sents))

# 重新初始化数据集
trainData = idData(tokenizer, filtered_train_sents, label2id)


if args.reload:
    print('in')
    check_point = torch.load(model_dir + args.model + ".model",map_location='cuda:2')
