...
parser.add_argument("-s", "--savemodel", help="save model during evaluation", action="store_true", default=True)
...
def run_epoch(data_iter, model, loss_compute,train=True,id2label=None):
    ...
    for i, (sent_batch,tag_batch) in enumerate(data_iter):
        ...
        if train:
            ...
            for i in range(len(y) // aveLength):
                tagSents.append([id2label[item] for item, yi in zip(results[i * aveLength:(i + 1) * aveLength],
                                                                    y[i * aveLength:(i + 1) * aveLength]) if not yi == pad])
    ...
    # 在评估模式下，根据评估指标筛选出质量更好的句子
    if not train:
        threshold = 0.8  # 设置阈值，根据实际情况调整
        filtered_data = [(sent_batch, tag_batch) for f1_score, (sent_batch, tag_batch) in zip(tagSents, data_iter) if f1_score >= threshold]

        return total_loss / total_tokens,f,tagSents,filtered_data
...
def run(check_point,epoch,pretrain_epoch,model,pretrain_batch_size,batch_size,pretrainData,trainData,valData,testData,id2label,w_padding,test=False):
    ...
    if test:
        ...
        loss, f, _ = run_epoch(batchIter(valData, batch_size, w_tag_pad=w_padding, t_tag_pad=len(id2label)), model,
                            SimpleLossCompute(criterion, optimizer, scheduler), train=False, id2label=id2label)
        print('Loss:', loss)
        print('Evaluation_test: ')
        loss, f, _ = run_epoch(batchIter(testData, batch_size, w_tag_pad=w_padding, t_tag_pad=len(id2label)), model,
                            SimpleLossCompute(criterion, optimizer, scheduler), train=False, id2label=id2label)
        print('Loss:', loss)
        return
    for i in range(epoch):
        ...
        loss,f,testTag, testData=run_epoch(batchIter(testData, batch_size, w_tag_pad=w_padding,t_tag_pad=len(id2label)), model,
                                           SimpleLossCompute(criterion,  optimizer,scheduler), train=False, id2label=id2label)
        print('Loss:', loss)
